...
funi_raccoon_game_location = {}
funi_raccoon_game_location.__index = funi_raccoon_game_location
...
function funi_raccoon_game_location.new(name)
    local self = setmetatable({}, funi_raccoon_game_location)
    if name then
...
function funi_raccoon_game_location:connect_one_way(exit, rule)
    if type(exit) == "string" then
        ...
        if existing then
            ...
            exit = funi_raccoon_game_location.new(exit)
        end
    end
    if rule == nil then
        rule = always
...
function funi_raccoon_game_location:connect_two_ways(exit, rule)
...
function funi_raccoon_game_location:connect_one_way_entrance(name, exit, rule)
    if rule == nil then
        rule = always
    end
    self.exits[#self.exits + 1] = { exit, rule }
...
function funi_raccoon_game_location:connect_two_ways_entrance(name, exit, rule)
...
-- acts as a shortcut for 2 connect_one_way_entrance-calls
function funi_raccoon_game_location:connect_two_ways_entrance_door_stuck(name, exit, rule1, rule2)
...
function funi_raccoon_game_location:connect_two_ways_stuck(exit, rule1, rule2)
...
function funi_raccoon_game_location:accessibility()
...
-- 
function funi_raccoon_game_location:discover(accessibility, keys)
...
entry_point = funi_raccoon_game_location.new("entry_point")
